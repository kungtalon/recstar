...
class YoutubeNet:
    ...
    def build_model(self):
        #placeholder
        self.hist_i = tf.placeholder(tf.int32, [None, None]) # history order[B, T]
        ...
        aisle_emb_w = tf.get_variable("aisle_emb_w", [self.aisle_count, self.embedding_size])
        dept_emb_w = tf.get_variable("dept_emb_w", [self.dept_count, self.embedding_size])
        ...
        if self.is_training:
            ...
            all_emb=tf.concat([item_emb_w,
                            tf.nn.embedding_lookup(aisle_emb_w,aisle_list),
    ...
    def train(self, sess, data, l):
        loss, _= sess.run([self.loss,self.train_op], feed_dict={
            self.sub_sample: data['sub_sample'],
            self.y: data['y'],
...
class YoutubeNet_reorder:
    ...
    def build_model(self):
        #placeholder
        self.hist_i = tf.placeholder(tf.int32, [None, None]) # history order[B, T1]
        self.hist_r = tf.placeholder(tf.int32, [None, None]) # history reorder[B, T2]
        self.sl = tf.placeholder(tf.int32, [None,]) # history len [B]
        self.basic=tf.placeholder(tf.float32,[None,None]) #user basic feature[B,basic_size]
        self.sub_sample = tf.placeholder(tf.int32, [None, None])  # soft layer (pos_clict,neg_list)[B,sub_size]
        self.y = tf.placeholder(tf.float32, [None, None])  # label one hot[B]
